 ...
 *
 * Exported so the pre-flight check and the admin can report the truth rather
 * than discovering it when a teacher uploads their first photograph.
 */
...
export function getMediaStore(): MediaStore {
  ...
  /*
    On an ephemeral host the local adapter is NOT offered, even though it would
    appear to work for the length of one deploy. A photograph that uploads
    successfully, displays correctly, and vanishes at the next deploy is worse
    than an upload that refuses outright: the first loses data quietly, and
    nobody finds out until a parent asks where their child's photo went.
  */
  store =
    hostDiscardsItsDisk() && !mediaStorageIsProductionReady()
